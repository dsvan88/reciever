...
$action->query(
    str_replace('{TABLE_MAIN}', TABLE_MAIN,
        "CREATE TABLE IF NOT EXISTS {TABLE_MAIN} (
            ...
            source CHARACTER VARYING(150) NOT NULL DEFAULT '',
...
$action->query(
    str_replace('{TABLE_CONTACTS}', TABLE_CONTACTS,
        "CREATE TABLE IF NOT EXISTS {TABLE_CONTACTS} (
            id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            uid INT NOT NULL DEFAULT '0',
            type CHARACTER VARYING(20) NOT NULL DEFAULT '',
            value CHARACTER VARYING(150) NOT NULL DEFAULT ''
        );"
    )
);
$action->query(
    str_replace('{TABLE_NOTES}', TABLE_NOTES,
        "CREATE TABLE IF NOT EXISTS {TABLE_NOTES} (
            id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            mid INT NOT NULL DEFAULT '0',
            uid INT NOT NULL DEFAULT '0',
            test CHARACTER VARYING NOT NULL DEFAULT '',
            time INT NOT NULL DEFAULT '0'
        );"
    )
);
...
if (isset($_POST['login']) && trim($_POST['login']) !== ''){
    if (!$action->recordExists(['id'=>1],TABLE_USERS)){
        $action->rowInsert([
            ...
            'password' => trim($_POST['password']) !=='' ? password_hash(sha1(trim($_POST['password'])),PASSWORD_DEFAULT) : '$2y$10$nrFt674uOjlgALrZBfu4nu6a4f8bX1h/rmoe2A.2kPUNspmObnl4q',
            'role' => 'admin'
        ],TABLE_USERS);
    }
